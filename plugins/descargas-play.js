...
const handler = async (m, { conn, text, usedPrefix, command }) => {
  try {
    ...
    // 🎥 VIDEO (Nexfuture API)
      ...
      try {
        const response = await fetch(`https://api.nexfuture.com.br/playvideo?id=${ytplay2.videoId}`)
        const json = await response.json()
        const result = json.resultado?.video
        const titulo = json.resultado?.titulo || title || "Desconocido"

        if (!result) throw new Error('⚠ El enlace de video no se generó correctamente.')
        await conn.sendFile(m.chat, result, `${titulo}.mp4`, titulo, m)
      } catch (e) {
